...
| First-run experience | A new visitor needed private audio files before hearing the idea. | **Load instant demo** now synthesizes two short loops locally, then **Start performance** launches music and hand controls together. | `src/lib/demoAudio.ts`, the First Mix director, and deterministic browser tests |
...
The current launch candidate has 164 passing unit and integration tests with 85.53% statement, 79.24% branch, 82.05% function, and 87.7% line coverage. A single-build browser gate exercises the production MediaStream and MediaPipe lifecycle, generated open-palm pickup, loaded-demo responsiveness, mobile reflow, keyboard focus, and WCAG A/AA checks. The same launch gate also passes lint, the production build, the enforced initial-JavaScript budget, and the full dependency audit with zero known vulnerabilities.
